        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                    <div class="p-2 bg-info bg-opacity-10 text-info rounded me-2">
                        <i class="fa-solid fa-pen-to-square fa-lg"></i>
                    </div>
                    <div>
                        <h4 class="card-title mb-0 font-weight-bold text-dark">কারিগর মজুরি হার সম্পাদনা করুন</h4>
                        <small class="text-muted">আইডি: #{{ $mojuri->id }} &bull; {{ $mojuri->category_name }} ({{ strtoupper($mojuri->type) }})</small>
                    </div>
                </div>
            ...
            <div class="card-body p-4">
                <form action="{{ route('karigor-mojuri.update', $mojuri->id) }}" method="POST" id="mojuriEditForm">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="input-group">
                                ...
                                <select name="category_name" id="category_name" class="form-select @error('category_name') is-invalid @enderror" required>
                                    @foreach($categories as $cat)
                                        <option value="{{ $cat }}" {{ (old('category_name', $mojuri->category_name) == $cat) ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <select name="type" id="type" class="form-select @error('type') is-invalid @enderror" required>
                                    @foreach($types as $t)
                                        <option value="{{ $t }}" {{ (strtolower(old('type', $mojuri->type)) == strtolower($t)) ? 'selected' : '' }}>
                                            {{ strtoupper($t) }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="p-3 bg-light rounded border mb-4">
                            ...
                            <div class="col-md-6 mb-3 mb-md-0">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" min="0" name="per_vori_tk" id="per_vori_tk" 
                                           ...
                                           placeholder="যেমন: ১৫০০.০০" value="{{ old('per_vori_tk', $mojuri->per_vori_tk) }}" required autofocus>
                                </div>
                                <small class="text-muted">ভরি হিসেবে কারিগরের মজুরি পরিবর্তন করুন</small>
                                @error('per_vori_tk')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" min="0" name="per_gram_tk" id="per_gram_tk" 
                                           ...
                                           placeholder="0.00" value="{{ old('per_gram_tk', $mojuri->per_gram_tk) }}" readonly>
                                </div>
                        ...
                        <div class="mt-3 p-2 bg-white rounded border d-flex justify-content-between align-items-center" id="calculationPreview">
                            <span class="small text-muted">
                                <i class="fa-solid fa-circle-info text-info me-1"></i>বর্তমান হিসাব:
                            </span>
                            <span class="small font-weight-bold text-dark" id="previewText">
                                ৳ {{ number_format($mojuri->per_vori_tk, 2) }} / ভরি &nbsp;&rarr;&nbsp; <span class="text-primary">৳ {{ number_format($mojuri->per_gram_tk, 2) }} / গ্রাম</span>
                            </span>
                        </div>
                    </div>
