    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <h3><i class="fa-solid fa-store me-2 text-primary"></i>দোকান স্টক তালিকা</h3>
                <span class="badge bg-primary fs-6">মোট পণ্য: {{ $purchases->count() }}</span>
            </div>
        </div>
        <div class="card-body">

            {{-- পণ্য অনুযায়ী সারসংক্ষেপ --}}
            @php
                $productSummary = $purchases->groupBy(fn($p) => $p->product->product_name ?? '—');
            @endphp
            @if($productSummary->count())
            <div class="mb-4">
                <div class="d-flex align-items-center gap-2 mb-2">
                    <i class="fa-solid fa-cubes text-primary"></i>
                    <span class="fw-semibold text-secondary" style="font-size:13px;">পণ্য অনুযায়ী সারসংক্ষেপ</span>
                </div>
                <div class="d-flex flex-wrap gap-3">
                    @foreach($productSummary as $productName => $items)
                    @php
                        $totalGram = $items->sum('gram');
                        $itemCount = $items->count();
                    @endphp
                    <div class="rounded-3 border px-3 py-2 d-flex flex-column gap-1"
                         style="min-width:200px;background:linear-gradient(135deg,#f8f9ff,#eef2ff);border-color:#c7d2fe!important;">
                        <div class="d-flex align-items-center gap-2">
                            <span class="badge bg-info fs-6 px-3 py-1" style="letter-spacing:.3px;">
                                <i class="fa-solid fa-box fa-xs me-1"></i>{{ $productName }}
                            </span>
                        </div>
                        <div class="d-flex gap-2 mt-1 flex-wrap">
                            <span class="badge" style="background:#6366f1;font-size:11px;">
                                <i class="fa-solid fa-hashtag fa-xs me-1"></i>
                                মোট আইটেম: {{ $itemCount }}
                            </span>
                            <span class="badge bg-success" style="font-size:11px;">
                                <i class="fa-solid fa-scale-balanced fa-xs me-1"></i>
                                মোট ওজন: {{ number_format($totalGram, 3) }} গ্রাম
                            </span>
